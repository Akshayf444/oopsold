...
if (isset($_POST['submit'])) {
    ...
    $found_user = BM::authenticate($username, $password);
    if ($found_user) {
        $changePassword = BM::changePassword($newPassword, $username);
        if ($changePassword) {
...
if (isset($_POST['upload']) && $_POST['upload'] != '') {
    if (isset($_FILES['file']) && $_FILES['file']['name'] != "") {
        ...
        $extension = end($temp);
        if ((($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "image/png") || ($_FILES["file"]["type"] == "image/pjpeg")) && ($_FILES["file"]["size"] < 200000) && in_array($extension, $allowedExts)) {
            if ($_FILES["file"]["error"] > 0) {
                ...
                $Employee = BM::updatepic($empid, $fileName);
            }
        } else {
...
$empName = BM::find_by_bmid($empid);
?>
<?php require_once("BMheader.php"); ?>
<div class="row">
    <div class="col-lg-12">
            ...
            <div class="tab-pane fade in active result" id="home-pills" style="margin-top: 1em">
                ...
                <table cellspacing="0" class="table table-bordered">
                    ...
                    <tr>
                        <th>Reporting SM</th>
                        <td><?php
                            $SM = SM::find_by_smid($empName->sm_empid);
                            if (isset($SM->name)) {
                                echo $SM->name;
                            }
                            ?></td>
